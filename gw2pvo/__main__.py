...
def run_once(settings, city):
    ...
    if data['pgrid_w'] == 0 and abs(eday_kwh - last_eday_kwh) < 0.001:
        logging.debug("Ignore unchanged pv reading")
    else:
        last_eday_kwh = eday_kwh

    if data['load'] == 0 and abs(energy_used - last_energy_used) < 0.001:
        logging.debug("Ignore unchanged load reading")
    else:
...
def copy(settings):
    ...
    if settings.pvo_system_id and settings.pvo_api_key:
        ...
        #    temperatures = temperatures.replace('dt','time')
        #    temperatures = temperatures.replace('temp','temperature')
        ...
        pvo = pvo_api.PVOutputApi(settings.pvo_system_id, settings.pvo_api_key)
        logging.debug("DATA ENTRIES:" + str(data['entries']))
        logging.debug("TEMPS: " + str(temperatures))
